...
export default function SideBar( {isOpenSide, setIsOpenSide}) {
  ...
  return (
    <div className={`h-screen ${isOpenSide ? ' w-64' : 'w-20'} bg-purple-800 text-white flex flex-col justify-between transition-all duration-300`}>
      ...
      <div>
        ...
        <nav className='space-y-1 mt-4 px-2'>
          <Link to="/adminDashboard" className={`flex items-center text-white px-4 py-3 rounded-md ${location.pathname === "/adminDashboard" ?
            ' bg-zinc-600 ' : 'hover: bg-zinc-400 text-white'} `} onClick={() => setIsOpenSide(false)}>
            { isOpenSide ? (
          ...
          <Link to="/admin/benevoles" className={`flex items-center text-white px-4 py-3 rounded-md ${location.pathname === "/admin/benevoles" ?
            ' bg-zinc-600 ' : 'hover:bg-gray-500 text-white'} `} onClick={() => setIsOpenSide(false)}>
            { isOpenSide ? (
              <div className='flex items-center'>
      ...
      <div className='  border-gray-400'></div>
            ...
            ) : (<LucideArrowLeftToLine className='size-6  mx-2' />)
            }
            </button>
      </div>
