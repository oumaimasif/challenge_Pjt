...
const GestionBenevole = () => {
  ...
  useEffect(() => {
    ...
    if (user.role !== "benevole" || user.id !== id) {
    ...
    const fetchAnnonces = async () => {
      try {
        ...
        const response = await axios.get(
          `http://localhost:5000/annonces/allAnnoncesByBenevole/${id}`,
          {
  ...
  const handleDelete = async (annonceId) => {
    if (window.confirm("Êtes-vous sûr de vouloir supprimer cette annonce?")) {
      try {
        ...
        toast.success("Annonce bien supprimée")
      } catch (error) {
  ...
  return (
    <div className="md:mx-14 lg:mx-20 pt-28 px-4 pb-10">
      <div className="flex justify-between items-center flex-wrap gap-4 mb-6">
        <div>
          <h1 className="text-3xl font-bold text-gray-800">Gestion de mon compte bénévole</h1>
          <TodayDate />
        </div>
        <Link
      ...
      <div className="bg-blue-200 rounded-lg shadow-md p-6 mb-10">
        <h2 className="text-xl font-semibold text-gray-800 mb-4">Informations de mon compte</h2>
        <p className="text-gray-600 mb-4">Vous pouvez mettre à jour vos informations personnelles et les détails de votre profil bénévole.</p>
        <Link
          to={`/editBenevole/${id}`}
...
export default GestionBenevole;