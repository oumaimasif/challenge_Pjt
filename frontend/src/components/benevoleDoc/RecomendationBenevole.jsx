...
const RecomendationBenevole = () => {
  const { id: sendToId } = useParams();  // l'ID du bénévole
  ...
  useEffect(() => {
    const fetchRecs = async () => {
      try {
        const res = await axios.get(`http://localhost:5000/recommendation/benevole/${sendToId}`);
        setRecs(res.data);
      } catch (err) {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      await axios.post(
        "http://localhost:5000/recommendation/add",
        { sendToId, roleType: "benevole", content, score },
        {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      // Filtrer la recommandation supprimée
  ...
  if (loading) {
    return (
      <div className='bg-white mt-4 shadow-lg w-[340px] md:w-[850px] relative -top-2 rounded-lg p-6'>
  ...
  return (
    <div className='bg-white mt-4 shadow-lg w-[340px] md:w-[850px] relative -top-2 rounded-lg p-6'>
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-violet-700">Recommandations</h2>
        {user && (
          hasUserRecommended ? (
      ...
      {recs.length === 0 ? (
        ...
        <div className="space-y-4 overflow-y-auto" style={{ maxHeight: "60vh" }}>
          {recs.map((rec) => (
            <div key={rec._id}
              className={`border relative rounded-lg p-4 ${rec.authorId === (user?.id || '') ? 'bg-orange-50 border-orange-200' : 'bg-gray-50 border-gray-200'}`} >
              <MdOutlineRateReview className='absolute right-5 bottom-2 text-orange-500' size={24} />
              ...
              <div className="flex items-center justify-between mb-3">
                    ...
                    <div className='flex space-x-1 mb-2'>
                      {[1, 2, 3, 4, 5].map(note => (
                        <FaStar key={note} size={16} className={`${note <= rec.score ?
                          "text-yellow-400" : "text-gray-300"} mr-1`} />
                      ))}
                    </div>
                ...
                <div className="text-xs px-2 py-1 bg-white text-orange-700 rounded-full">
            ...
            <div className='flex space-x-1 mb-4'>
              {[1, 2, 3, 4, 5].map(note => (
                <FaStar key={note} size={24} className={`${note <= score ?
                  "text-yellow-400" : "text-gray-300"} cursor-pointer`}
                  onClick={() => setScore(note)} />
              ))}
            </div>
            <textarea
              value={content}
              onChange={(e) => setContent(e.target.value)}
              placeholder="Partagez votre expérience avec ce bénévole..."
